...
void Simulator::Simulate(int argc, char **argv){
  ...
  if(bParams->iTrackOnly != 1){
    ...
    //!-------------------------------------------------------------------
    cout<<"Sim File:"<<bParams->Mp_file<<endl;
...
quad::timer::start_timer(&gpu_alloc);
	...
	*/
    ...
    /*if(world_rank == 0) 
	{
        std::cout<<"1000 Main array elements are \n";

        for(int i = 0; i < 120000; i++)
        {
            std::cout<<"i is "<<i<<" -- e-beam is "<<x_e[i]<<"\n";
        }
		cout<<"i is"<<(bParams->ebunches) * Npart_p * (NCOL + 2)-1<<(bParams->ebunches) * Npart_p * (NCOL + 2)-1<<" -- e-beam is "<<x_e[(bParams->ebunches) * Npart_p * (NCOL + 2)-1]<<"\n";
    }
	MPI_Barrier(MPI_COMM_WORLD);
	cout<<endl<<endl;
	if(world_rank == 0)
	{
		std::cout<<"Process 0 array elements are \n";
			
		for(int i = 0; i < 1000; i++) {
			std::cout<<"x_e_mp is of process 0 is "<<x_e_mp[i]<<"\n";
		}
	}
	MPI_Barrier(MPI_COMM_WORLD);
	cout<<endl<<endl;
	if(world_rank == 1)
	{
		std::cout<<"Process 1 array elements are \n";
			
		for(int i = 0; i < 1000; i++) {
			std::cout<<"x_e_mp is of process 1 is "<<x_e_mp[i]<<"\n";
		}
	}
	MPI_Barrier(MPI_COMM_WORLD);
	cout<<endl<<endl;
	if(world_rank == 2)
	{
		std::cout<<"Process 2 array elements are \n";
			
		for(int i = 0; i < 1000; i++) {
			std::cout<<"x_e_mp is of process 2 is "<<x_e_mp[i]<<"\n";
		}
	}
	MPI_Barrier(MPI_COMM_WORLD);
	cout<<endl<<endl;
	if(world_rank == 3)
	{
		std::cout<<"Process 3 array elements are \n";
			
		for(int i = 0; i < 1000; i++) {
			std::cout<<"x_e_mp is of process 3 is "<<x_e_mp[i]<<"\n";
		}
	}*/
	...
	MPI_Request myRequest_recv_e[10], myRequest_send_p[10];
	MPI_Status status_recv_e[10], status_send_p[10];
...
int POST_COLLIDE = 0;int POST_COLLIDE2 = 0;
	for(int iTurn = 1; iTurn <= bParams->Niter; ++iTurn)
	{
		//std::cout<<"Running for turn "<<iTurn<<"\n";
		total_ij = 0;
		for(int i = 0; i < num_bunch_p; i++)
        {
			...
			Lum = 0.0;
			//if(world_rank ==0)
                //cout<<"Current Schedule is "<<"\n";
            for(int j = 0; j < num_bunch_e; j++)//current schedule
            {
                ...
                //if(world_rank ==0)
					//std::cout<<current_e_schedule[j]<<" -- "<<current_p_schedule[j]<<"\n";
                total_ij++;
            }
			//cout<<"max_bunches_per_gpu_p:"<<max_bunches_per_gpu_p<<endl;	
            for(int timestamp = 0; timestamp < max_bunches_per_gpu_p; timestamp++)
            {
                ...
                for(int j = 0; j < num_gpu; j++)//sub-schedule
                {
                    sub_e_schedule[j] = -1;
                    sub_p_schedule[j] = -1;
                    gpu_occ_e[j] = -1;
					...
					//if(world_rank == 0)
						//cout<<"bunch count of gpu["<<j<<"]"<<bunch_count_of_gpu_p[j]<<endl;
					
                    if(bunch_offset < num_bunch_e && timestamp < bunch_count_of_gpu_e[j]) //we grab what collision we want to do for each GPU without looking at resource contention
					{
						//if(world_rank == 0)
							//std::cout<<"Time:"<<timestamp<<"| "<<current_e_schedule[bunch_offset]<<"-"<<current_p_schedule[bunch_offset]<<"("<<j<<")\n";
                ...
                {
					//cout<<"Remaining bunches:"<<rem_bunch<<"("<<world_rank<<")\n";
                    for(int rb = 0; rb < rem_bunch; rb++)//we first look at the bunches in the queue remaining bunches
                    {
                    ...
                    for(int j = 0; j < num_gpu; j++)
                    {
                        if(sub_e_schedule[j] != -1 && sub_p_schedule[j] != -1)
                        {
							...
							if(world_rank ==0)
							{
								//cout<<"gpu_occupanices:"<<gpu_occ_e[gpu_num_e]<<"&"<<gpu_occ_p[gpu_num_p]<<" ("<<j<<")\n";
								//cout<<"sub_schedules:"<< sub_e_schedule[j]<<"&"<< sub_p_schedule[j]<<" ("<<j<<")\n";
							}
...
quad::timer::event_pair timer_post_col2;
				if(bParams->isGPU)
				{
					...
					/*//printf("opp_bunch_gpu: %i-%i (%i)\n", opp_e_bunch_gpu, opp_p_bunch_gpu, world_rank);
					//printf("final bunches of gpu: %i-%i (%i)\n", final_e_bunch_of_gpu[world_rank], final_p_bunch_of_gpu[world_rank], world_rank);
					//printf("opp_var: %i-%i (%i)\n", opp_e_bunch_var, opp_p_bunch_var, world_rank);
					//if(opp_e_bunch_gpu == world_rank)//means this GPU is providing the e-beam
						//cout<<"opp_var:"<<starting_bunch_of_gpu_e[world_rank]+my_e_bunch_offset;
					//else
						//cout<<"opp_var:"<<opp_e_bunch_var;
					//cout<<"-";
					//if(opp_p_bunch_gpu == world_rank)//means this GPU is providing the e-beam
						//cout<<starting_bunch_of_gpu_p[world_rank]+my_p_bunch_offset<<endl;
					//else
						//cout<<opp_p_bunch_var<<endl;
					
					/*if(final_e_bunch_of_gpu[world_rank]!=-1 || final_p_bunch_of_gpu[world_rank]!=-1)
					{
						cout<<"Coll:";
						if(final_e_bunch_of_gpu[world_rank]!=-1)
							cout<<final_e_bunch_of_gpu[world_rank];
						else
							cout<<opp_e_bunch[final_p_bunch_of_gpu[world_rank]];
						cout<<"-";
						if(final_p_bunch_of_gpu[world_rank]!=-1)
							cout<<final_p_bunch_of_gpu[world_rank]<<endl;
						else
							cout<<opp_p_bunch[final_e_bunch_of_gpu[world_rank]]<<endl;
					}*/
					//printf("my_bunch_offset: %i-%i (%i)\n", my_e_bunch_offset, my_p_bunch_offset, world_rank);
					...
					collide->collide(lum_turn, &dOutOfBound_e_mp[my_e_bunch_offset * Npart_e], &dOutOfBound_p_mp[my_p_bunch_offset * Npart_p], &dx_e_mp[my_e_bunch_offset * Npart_e * (NCOL + 2)], &dx_p_mp[my_p_bunch_offset * Npart_p * (NCOL + 2)], bParams, final_e_bunch_of_gpu[world_rank], opp_p_bunch_var, opp_p_bunch_gpu, final_p_bunch_of_gpu[world_rank], opp_e_bunch_var, opp_e_bunch_gpu);
...
quad::timer::start_timer(&timer_post_col2);
				if(final_e_bunch_of_gpu[world_rank] != -1)
				{ 
					//cout<<"Computing RMS:"<<world_rank<<endl;
					computeRMS(&dx_e_mp[my_e_bunch_offset * Npart_e * (NCOL + 2)], bParams->Npart_e,
			               		xbar_e, ybar_e, zbar_e, sig_x_e, sig_y_e, sig_z_e,
               					mom_x_e, mom_y_e, mom_z_e,
		        	       		pxbar_e, pybar_e, pzbar_e, sig_px_e, sig_py_e, sig_pz_e, bParams->Npart_inbound_e);
				}

				if(final_p_bunch_of_gpu[world_rank] != -1)
				{//cout<<"Computing RMS:"<<world_rank<<endl;
		    		computeRMS(&dx_p_mp[my_p_bunch_offset * Npart_p * (NCOL + 2)], bParams->Npart_p,
			        	       xbar_p, ybar_p, zbar_p, sig_x_p, sig_y_p, sig_z_p,
				               mom_x_p, mom_y_p, mom_z_p,
				               pxbar_p, pybar_p, pzbar_p, sig_px_p, sig_py_p, sig_pz_p, bParams->Npart_inbound_p);
				}
MPI_Barrier(MPI_COMM_WORLD);			
...
POST_COLLIDE2 = temp_post_col2 + POST_COLLIDE2;				
				if(final_p_bunch_of_gpu[world_rank] != -1)
				{
					...
					{
						MPI_Isend(&sig_x_p, 1, MPI_DOUBLE, opp_e_bunch_gpu, 0, MPI_COMM_WORLD, &myRequest_send_p[0]);
						MPI_Isend(&sig_y_p, 1, MPI_DOUBLE, opp_e_bunch_gpu, 1, MPI_COMM_WORLD, &myRequest_send_p[1]);
                                                MPI_Isend(&xbar_p, 1, MPI_DOUBLE, opp_e_bunch_gpu, 2, MPI_COMM_WORLD, &myRequest_send_p[2]);
                                                MPI_Isend(&ybar_p, 1, MPI_DOUBLE, opp_e_bunch_gpu, 3, MPI_COMM_WORLD, &myRequest_send_p[3]);
                                                MPI_Isend(&sig_px_p, 1, MPI_DOUBLE, opp_e_bunch_gpu, 4, MPI_COMM_WORLD, &myRequest_send_p[4]);
                                                MPI_Isend(&sig_py_p, 1, MPI_DOUBLE, opp_e_bunch_gpu, 5, MPI_COMM_WORLD, &myRequest_send_p[5]);
                                                MPI_Isend(&momxp0, 1, MPI_DOUBLE, opp_e_bunch_gpu, 6, MPI_COMM_WORLD, &myRequest_send_p[6]);
                                                MPI_Isend(&momxp1, 1, MPI_DOUBLE, opp_e_bunch_gpu, 7, MPI_COMM_WORLD, &myRequest_send_p[7]);
                                                MPI_Isend(&momyp0, 1, MPI_DOUBLE, opp_e_bunch_gpu, 8, MPI_COMM_WORLD, &myRequest_send_p[8]);
                                                MPI_Isend(&momyp1, 1, MPI_DOUBLE, opp_e_bunch_gpu, 9, MPI_COMM_WORLD, &myRequest_send_p[9]);
                                        }
				}
				...
				if(final_e_bunch_of_gpu[world_rank] != -1)
				{
					...
					{
						MPI_Irecv(&sig_x_p_rcvd, 1, MPI_DOUBLE, opp_p_bunch_gpu, 0, MPI_COMM_WORLD, &myRequest_recv_e[0]);
						MPI_Irecv(&sig_y_p_rcvd, 1, MPI_DOUBLE, opp_p_bunch_gpu, 1, MPI_COMM_WORLD, &myRequest_recv_e[1]);
                                                MPI_Irecv(&xbar_p_rcvd, 1, MPI_DOUBLE, opp_p_bunch_gpu, 2, MPI_COMM_WORLD, &myRequest_recv_e[2]);
                                                MPI_Irecv(&ybar_p_rcvd, 1, MPI_DOUBLE, opp_p_bunch_gpu, 3, MPI_COMM_WORLD, &myRequest_recv_e[3]);
                                                MPI_Irecv(&sig_px_p_rcvd, 1, MPI_DOUBLE, opp_p_bunch_gpu, 4, MPI_COMM_WORLD, &myRequest_recv_e[4]);
                                                MPI_Irecv(&sig_py_p_rcvd, 1, MPI_DOUBLE, opp_p_bunch_gpu, 5, MPI_COMM_WORLD, &myRequest_recv_e[5]);
                                                MPI_Irecv(&momxp0, 1, MPI_DOUBLE, opp_p_bunch_gpu, 6, MPI_COMM_WORLD, &myRequest_recv_e[6]);
                                                MPI_Irecv(&momxp1, 1, MPI_DOUBLE, opp_p_bunch_gpu, 7, MPI_COMM_WORLD, &myRequest_recv_e[7]);
                                                MPI_Irecv(&momyp0, 1, MPI_DOUBLE, opp_p_bunch_gpu, 8, MPI_COMM_WORLD, &myRequest_recv_e[8]);
                                                MPI_Irecv(&momyp1, 1, MPI_DOUBLE, opp_p_bunch_gpu, 9, MPI_COMM_WORLD, &myRequest_recv_e[9]);
				...
				if(final_e_bunch_of_gpu[world_rank] != -1)
				{
					...
					if(world_rank != 0)
						MPI_Send(&Lum, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
				}
				...
				if(world_rank == 0)
				{
					...
					for(int i = 1; i < world_size; i++)
					{
						...
						{
							MPI_Recv(&Lum_rcvd, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
						//std::cout<<"Lum_Rcvd is "<<Lum_rcvd<<"\n";
							Lum_total += Lum_rcvd;
						}
					}
				}
...
temp_post_col2 = 0;
			...
			if(world_rank == 0)
			{
				...
				//cout<<"Lum_turn "<<lum_turn<<endl;
                          io->dumpLum(lum_turn, Lum_total/num_bunch_p, bParams->Lc, bParams->Lsl, bParams->N, xbar_e, ybar_e, xbar_p_rcvd, ybar_p_rcvd, sig_x_e, sig_y_e,
                                      sig_x_p_rcvd, sig_y_p_rcvd, mom_x_e, mom_y_e, mom_x_p_rcvd, mom_y_p_rcvd,
